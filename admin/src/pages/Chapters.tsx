...
const Chapters = () => {
  ...
  useEffect(() => {
    if (!selectedRoadmapId && roadmaps?.[0]?.id) {
      setSelectedRoadmapId(roadmaps[0].id);
      setChapterForm((prev) => ({ ...prev, roadmap_id: roadmaps[0].id }));
    }
  }, [roadmaps, selectedRoadmapId]);

  const chaptersQuery = useQuery({
    queryKey: ['admin-chapters', selectedRoadmapId],
    queryFn: () => chaptersAdminService.list(selectedRoadmapId),
    enabled: Boolean(selectedRoadmapId),
  });
  ...
  useEffect(() => {
    if (!selectedChapterId) {
      setChapterForm((prev) => ({
        ...EMPTY_FORM,
        roadmap_id: selectedRoadmapId || prev.roadmap_id,
      }));
      setContentJson('{\n  "video_youtube_id": "",\n  "video_title": "",\n  "quiz": [],\n  "tasks": []\n}');
      setStepDrafts([]);
      return;
    ...
    setChapterForm({
      roadmap_id: String(detail.chapter.roadmap_id || selectedRoadmapId),
  ...
  }, [detailQuery.data, selectedChapterId, selectedRoadmapId]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (chapter: AdminChapter) => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['admin-chapters', chapter.roadmap_id] });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['admin-chapters', selectedRoadmapId] });
  ...
  const saveChapter = () => {
    if (!chapterForm.roadmap_id || !chapterForm.title.trim()) {
      toast.error('Roadmap and title are required');
      return;
    }
        ...
        <Card className="border-0 shadow-md">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-xs">Roadmap</Label>
              <Select
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder={roadmapsLoading ? 'Loading roadmaps...' : 'Select roadmap'} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              {chaptersQuery.isLoading ? (
                ...
                <div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
                  No chapters yet for this roadmap.
                </div>
              ) : (
        ...
        <div className="space-y-6">
          <Card className="border-0 shadow-md">
            ...
            <CardContent className="grid gap-4 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">Roadmap</Label>
                <Input value={selectedRoadmap?.title || 'Select a roadmap'} disabled />
              </div>
              <div className="space-y-1.5">
