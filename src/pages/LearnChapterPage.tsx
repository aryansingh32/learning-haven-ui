...
import { VisualizerSection } from '@/components/chapter/VisualizerSection';
import { DocSection } from '@/components/chapter/DocSection';
import { CompleteStepSection } from '@/components/chapter/CompleteStepSection';
import { MicroRevisionSection } from '@/components/chapter/MicroRevisionSection';
import CelebrationOverlay from '@/components/chapter/CelebrationOverlay';
...
export default function LearnChapterPage() {
  ...
  const roadmapTitle = data?.roadmap?.title || 'Learning Path';
  ...
  if (isLocked) {
    return (
      <motion.div className="max-w-7xl mx-auto card-layer-2 rounded-2xl p-8 text-center space-y-4">
        ...
        <p className="text-muted-foreground text-sm">Complete the previous chapter to unlock this one.</p>
        <ChapterCta variant="primary" onClick={() => navigate('/chapters')}>
          Back to roadmap
        </ChapterCta>
      </motion.div>
    );
        ...
        <div className="relative z-10 flex flex-wrap items-start justify-between gap-6">
          <div className="flex-1 min-w-[280px]">
            <div className="text-[10px] font-bold tracking-widest uppercase text-white/50 mb-3">
              {roadmapTitle.toUpperCase()} • CHAPTER {chapter.chapter_number}
            </div>
