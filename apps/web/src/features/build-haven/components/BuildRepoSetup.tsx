...
export function BuildRepoSetup({ repoUrl, setupInstructions, isGithubConnected, onConnectGithub, onCopy }: Props) {
  if (!repoUrl) {
    return (
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-border/60 bg-card/80 p-5 shadow-sm">
      ...
      <div className="space-y-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">1. Connect GitHub</p>
          ...
          <div className="flex items-center gap-2 text-sm">
            {isGithubConnected ? (
              <>
                ...
                <span className="font-medium text-foreground">GitHub account connected</span>
              </>
            ) : (
              <>
                <AlertCircle className="h-4 w-4 text-destructive" />
                <span className="font-medium text-foreground">GitHub account not connected</span>
              </>
            )}
          </div>
          {!isGithubConnected && (
      ...
      <div className="space-y-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">2. Clone repository</p>
      ...
      {setupInstructions ? (
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">3. Setup environment</p>
      ...
      <div className="space-y-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          {setupInstructions ? '4. Run & submit' : '3. Run & submit'}
        </p>
