...
import { BuildStageSidebar } from '@/features/build-haven/components/BuildStageSidebar';
...
import { StagePassModal } from '@/features/build-haven/components/StagePassModal';
...
export default function BuildWorkspacePage() {
  ...
  const [showLogsOpen, setShowLogsOpen] = useState(true);
  ...
  const [stagePassModalOpen, setStagePassModalOpen] = useState(false);
  const [passedStageForModal, setPassedStageForModal] = useState<number | null>(null);
  const [previousRank, setPreviousRank] = useState<number | null>(null);
  const [nextStageAlreadyImplemented, setNextStageAlreadyImplemented] = useState(false);
  ...
  const githubStatusQuery = useQuery({
    ...
    enabled: !!user,
    refetchInterval: viewModeState === 'setup' ? 5000 : false,
  });
  ...
  const baseVerdict = String(structured.verdict || lastResult?.status || 'pending');
  // Prevent "Waiting for your first push" on completed stages
  const verdict = stageStatusForView === 'completed' && baseVerdict === 'pending' ? 'passed' : baseVerdict;
  ...
  useEffect(() => {
    ...
    const unsubscribe = buildHavenService.subscribeToEnrollmentEvents(enrollment.id, (event) => {
      if (event.type === 'attempt_started' || event.type === 'verification_started' || event.type === 'verification_queued') {
        setIsVerifying(true);
        setLiveLogLines([]);
        setNextStageAlreadyImplemented(false);
        ...
        if (event.type === 'stage_result' && event.payload?.status === 'passed') {
          ...
          if (
            ...
            // Save previous rank before invalidation updates it
            setPreviousRank(userRank);
            // Show two-step completion modal instead of auto-confetti
            setPassedStageForModal(passedStageNum);
            setStagePassModalOpen(true);
          }
        }
        // Detect "Next stage already implemented" proactive state
        if (event.type === 'stage_result' && event.payload?.nextStageAlreadyImplemented) {
          setNextStageAlreadyImplemented(true);
        }
        invalidateWorkspace();
      }
    });
    return () => unsubscribe();
  }, [enrollment?.id, enrollment?.celebrated_stages, invalidateWorkspace, slug, userRank]);
  ...
  const disconnectGithubFromWorkspace = async () => {
    try {
      await apprenticeshipService.disconnectGithub();
  ...
  return (
    <div className="flex h-screen flex-col bg-depth text-foreground">
      ...
      <BuildWorkspaceTopBar
        ...
        onConnectGithub={connectGitHubFromWorkspace}
        onDisconnectGithub={disconnectGithubFromWorkspace}
      />
                ...
                {(!isVibeMode && !enrollment?.repo_url && !enrollment?.repo_full_name) && (
                  <div className="flex items-center gap-3 rounded-lg border border-warning/30 bg-warning/5 px-4 py-3">
                    ...
                    <p className="text-sm text-foreground">
                      <strong>Setup required:</strong> You must complete the{' '}
                      <button onClick={() => setViewMode('setup')} className="underline hover:text-warning transition-colors">
                        local setup
                      </button>{' '}
                      to connect GitHub before your pushes will be verified.
                    </p>
                  </div>
                )}
      ...
      {/* Two-step completion modal (CodeCrafters parity) */}
      <StagePassModal
        open={stagePassModalOpen}
        onOpenChange={setStagePassModalOpen}
        stageTitle={viewedStage?.title}
        stageNumber={passedStageForModal ?? undefined}
        newRank={userRank}
        previousRank={previousRank}
        onRefactor={() => {
          // User chose to refactor — close modal, stay on stage
          setStagePassModalOpen(false);
        }}
        onMarkComplete={() => {
          // User consciously clicked "Mark as Complete" — fire confetti now
          const stageNum = passedStageForModal;
          if (stageNum) {
            celebratedRef.current.add(stageNum);
            confetti({
              particleCount: 150,
              spread: 90,
              origin: { y: 0.6 },
            ...
            buildHavenService.celebrateStage(slug, stageNum).catch(() => {});
          }
          // Move to next stage after a short delay for the confetti to settle
          setTimeout(() => {
            invalidateWorkspace();
            if (nextStage) {
              setViewMode(nextStage.stage_number);
            }
          }, 800);
        }}
      />
