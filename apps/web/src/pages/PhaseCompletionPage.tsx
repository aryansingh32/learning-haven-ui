...
export default function PhaseCompletionPage() {
  ...
  const { data: serverPhases } = useQuery({
  ...
  const phase = phases.find((p: any) => p.id === phaseId) || phases[0];
  const currentIndex = phases.findIndex((p: any) => p.id === phaseId);
    ...
    <div className="max-w-7xl mx-auto space-y-6 py-4 pb-20 md:pb-8">
      ...
      >
        ...
        >
          ...
          <p className="text-sm opacity-70">Congratulations, {userName}! You've mastered this course.</p>
        </motion.div>
      </motion.div>
      ...
      {nextPhase ? (
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mb-4">{nextPhase.title}: {nextPhase.description}</p>
          <button
