...
export async function executeJava(code: string, testCases: TestCase[]): Promise<JavaExecutionResult> {
    ...
    try {
        ...
        if (hasSolutionClass && !hasMain) {
            ...
            await fs.writeFile(path.join(workDir, 'input.txt'), stdinData, 'utf8');

            try {
                const { stdout, stderr } = await execAsync(
                    `java -cp . ${runClass} < input.txt`,
                    {
                        cwd: workDir,
            ...
            for (let i = 0; i < testCases.length; i++) {
                ...
                await fs.writeFile(path.join(workDir, `input_${i}.txt`), tc.input || '', 'utf8');
                const start = Date.now();
                try {
                    const { stdout, stderr } = await execAsync(
                        `java -cp . ${runClass} < input_${i}.txt`,
                        {
                            cwd: workDir,
                            timeout: JAVA_TIMEOUT_MS,
                            env: { ...SANDBOX_ENV, JAVA_TOOL_OPTIONS: `-Xmx${JAVA_MEMORY_LIMIT}` }
