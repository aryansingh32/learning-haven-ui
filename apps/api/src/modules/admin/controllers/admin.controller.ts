...
import { TasksService } from '../../apprenticeship/services/tasks.service';
import { FeedbackService } from '../../learning/services/feedback.service';
import { JobsService } from '../../apprenticeship/services/jobs.service';
...
export class AdminController {
    ...
    static async importProblems(req: Request, res: Response) {
        try {
            ...
            if (sheetUrl) {
                const { GoogleSheetsService } = await import('../../core/services/googleSheets.service');
    ...
    static async listCourses(req: Request, res: Response) {
        try {
            const courses = await CoursesService.listCourses(true); // include unpublished
            res.json(courses);
        } catch (error) {
            logger.error('Admin list courses error:', error);
    ...
    static async createCourse(req: Request, res: Response) {
        try {
            const adminId = (req as AuthRequest).user!.id;
            const result = await CoursesService.createCourse(adminId, req.body);
            res.status(201).json(result);
        } catch (error) {
    ...
    static async updateCourse(req: Request, res: Response) {
        try {
            const result = await CoursesService.updateCourse(req.params.id as string, req.body);
            res.json(result);
        } catch (error) {
    ...
    static async reorderCourses(req: Request, res: Response) {
        try {
            const { courses } = req.body;
            if (!Array.isArray(courses)) {
                return res.status(400).json({ error: 'courses array is required' });
            }
            const result = await CoursesService.reorderCourses(courses);
            res.json({ message: 'Courses reordered', success: result });
        } catch (error) {
    ...
    static async deleteCourse(req: Request, res: Response) {
        try {
            const result = await CoursesService.deleteCourse(req.params.id as string);
            res.json(result);
        } catch (error) {
    ...
    static async addCourseItem(req: Request, res: Response) {
        try {
            ...
            logger.error('Add course item error:', error);
            res.status(500).json({ error: 'Failed to add course item' });
        }
    }

    static async removeCourseItem(req: Request, res: Response) {
        try {
            const result = await CoursesService.removeCourseItem(req.params.id as string, req.params.itemId as string);
            res.json(result);
        } catch (error) {
            logger.error('Remove course item error:', error);
            res.status(500).json({ error: 'Failed to remove course item' });
        }
    }

    static async reorderCourseItems(req: Request, res: Response) {
        try {
            const { items } = req.body;
            const result = await CoursesService.reorderItems(req.params.id as string, items);
            res.json(result);
        } catch (error) {
    ...
    static async listChapters(req: Request, res: Response) {
        try {
            ...
            if (!courseId) {
                return res.status(400).json({ error: 'course_id query required' });
            }
            const chapters = await AdminChaptersService.listByCourse(courseId);
            res.json({ chapters });
        } catch (error) {
    ...
    static async updateSettings(req: Request, res: Response) {
        try {
            ...
            for (const [key, value] of Object.entries(updates)) {
                ...
                if (error) {
                    logger.error(`Upsert failed for key: ${key}`, error);
                    throw error;
                }
            }
    ...
    static async updateLeaderboardConfig(req: Request, res: Response) {
        try {
            ...
            if (error) {
                logger.error('Leaderboard config upsert error:', error);
                throw error;
            }
    ...
    static async updateAIConfig(req: Request, res: Response) {
        try {
            ...
            for (const [key, value] of Object.entries(updates)) {
                ...
                if (error) {
                    logger.error(`Upsert failed for AI config key: ${key}`, error);
                    throw error;
                }
            }
