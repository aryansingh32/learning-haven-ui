...
DB_URL="${DATABASE_URL:?Set DATABASE_URL before running this script}"
JWT_SECRET="${JWT_SECRET:?Set JWT_SECRET before running this script}"
PGPASSWORD="${PGPASSWORD:?Set PGPASSWORD before running this script}"
...
psql "$DB_URL" -c "UPDATE users SET role = 'super_admin' WHERE id = '${USER_ID}';" > /dev/null 2>&1
...
psql "$DB_URL" -c "UPDATE users SET role = 'user' WHERE id = '${USER_ID}';" > /dev/null 2>&1
