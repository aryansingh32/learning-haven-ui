...
async function runSeeder() {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .upsert({
          ...
          order_index: roadmap.meta?.order ?? 0
        }, { onConflict: 'slug' });

      if (error) {
        console.error(`[ERROR] File ${file}: Failed to upsert course - ${error.message}`);
        errorCount++;
        continue;
      }

      console.log(`Course ${roadmap.title} — ✓ seeded`);
      seededCount++;
    } catch (err: any) {
  ...
  console.log(`\nDONE: ${seededCount} courses seeded, ${errorCount} errors`);
}
